...
from Bot.Infrastructure.DataBase.commands import append_message, get_messages, add_chat, drop_chat, get_flood_status, update_flood_status
...
async def message_read(message: types.Message):
    try:
        if message.reply_to_message is not None and message.text.lower() == "@violation_detect_bot":
            messages, ids = get_messages(connection, message.reply_to_message.from_user.id, message.reply_to_message.chat.id, is_deleted=0) #todo check for flood
            status, ids1 = compare_messages_to_flood_detect(dp, message.reply_to_message.chat.id, messages, ids)                                                                 #todo check images for flood
            if status:
                for id1 in ids1:
